class RubrosController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @rubro.save
        format.html { redirect_to rubro_url(@rubro), notice: "Rubro was successfully created." }
  ...
  def update
    respond_to do |format|
      if @rubro.update(rubro_params)
        format.html { redirect_to rubro_url(@rubro), notice: "Rubro was successfully updated." }
  ...
  private
    ...
    def rubro_params
      params.require(:rubro).permit(:identificacion_del_rubro, :identificacion_rubro_interno, :observacion, :codigo_dependencia, :nombres_del_rubro)
    end
end
